...
describe("timeago", () => {
  const ref = 1704067200; // 2024-01-01 00:00:00 UTC
  ...
  test("just now - 30 seconds", () => {
    expect(timeago(ref - 30, ref)).toBe("just now");
  });

  test("just now - 44 seconds", () => {
    expect(timeago(ref - 44, ref)).toBe("just now");
  });

  test("1 minute ago - 45 seconds", () => {
    expect(timeago(ref - 45, ref)).toBe("1 minute ago");
  });

  test("1 minute ago - 89 seconds", () => {
    expect(timeago(ref - 89, ref)).toBe("1 minute ago");
  });

  test("2 minutes ago - 90 seconds", () => {
    expect(timeago(ref - 90, ref)).toBe("2 minutes ago");
  });

  test("2 minutes ago - 150 seconds", () => {
    expect(timeago(ref - 150, ref)).toBe("3 minutes ago");
  });

  test("44 minutes ago", () => {
    expect(timeago(ref - 44 * 60, ref)).toBe("44 minutes ago");
  });

  test("1 hour ago - 45 minutes", () => {
    expect(timeago(ref - 45 * 60, ref)).toBe("1 hour ago");
  });

  test("1 hour ago - 89 minutes", () => {
    expect(timeago(ref - 89 * 60, ref)).toBe("1 hour ago");
  });

  test("2 hours ago - 90 minutes", () => {
    expect(timeago(ref - 90 * 60, ref)).toBe("2 hours ago");
  });

  test("21 hours ago", () => {
    expect(timeago(ref - 21 * 3600, ref)).toBe("21 hours ago");
  });

  test("1 day ago - 22 hours", () => {
    expect(timeago(ref - 22 * 3600, ref)).toBe("1 day ago");
  });

  test("1 day ago - 35 hours", () => {
    expect(timeago(ref - 35 * 3600, ref)).toBe("1 day ago");
  });

  test("2 days ago - 36 hours", () => {
    expect(timeago(ref - 36 * 3600, ref)).toBe("2 days ago");
  });

  test("25 days ago", () => {
    expect(timeago(ref - 25 * 86400, ref)).toBe("25 days ago");
  });

  test("1 month ago - 26 days", () => {
    expect(timeago(ref - 26 * 86400, ref)).toBe("1 month ago");
  });

  test("1 month ago - 45 days", () => {
    expect(timeago(ref - 45 * 86400, ref)).toBe("1 month ago");
  });

  test("2 months ago - 46 days", () => {
    expect(timeago(ref - 46 * 86400, ref)).toBe("2 months ago");
  });

  test("10 months ago - 319 days", () => {
    expect(timeago(ref - 319 * 86400, ref)).toBe("10 months ago");
  });

  test("1 year ago - 320 days", () => {
    expect(timeago(ref - 320 * 86400, ref)).toBe("1 year ago");
  });

  test("1 year ago - 547 days", () => {
    expect(timeago(ref - 547 * 86400, ref)).toBe("1 year ago");
  });

  test("2 years ago - 548 days", () => {
    expect(timeago(ref - 548 * 86400, ref)).toBe("2 years ago");
  });

  test("5 years ago", () => {
    expect(timeago(ref - 5 * 365 * 86400, ref)).toBe("5 years ago");
  });

  // Future times
  test("in 1 minute - 45 seconds", () => {
    expect(timeago(ref + 45, ref)).toBe("in 1 minute");
  });

  test("in 2 minutes - 90 seconds", () => {
    expect(timeago(ref + 90, ref)).toBe("in 2 minutes");
  });

  test("in 1 hour", () => {
    expect(timeago(ref + 3600, ref)).toBe("in 1 hour");
  });

  test("in 2 hours", () => {
    expect(timeago(ref + 2 * 3600, ref)).toBe("in 2 hours");
  });

  test("in 1 day", () => {
    expect(timeago(ref + 22 * 3600, ref)).toBe("in 1 day");
  });

  test("in 2 days", () => {
    expect(timeago(ref + 36 * 3600, ref)).toBe("in 2 days");
  });

  test("in 1 month", () => {
    expect(timeago(ref + 26 * 86400, ref)).toBe("in 1 month");
  });

  test("in 1 year", () => {
    expect(timeago(ref + 320 * 86400, ref)).toBe("in 1 year");
  });

  // Default reference (no second argument)
  test("default reference returns just now", () => {
    expect(timeago(ref)).toBe("just now");
  });
});
...
describe("duration", () => {
  ...
  test("30 days", () => {
    expect(duration(30 * 86400)).toBe("1 month");
  });

  test("365 days", () => {
    expect(duration(365 * 86400)).toBe("1 year");
  });

  test("1 year 6 months", () => {
    expect(duration(365 * 86400 + 180 * 86400)).toBe("1 year, 6 months");
  });

  // Compact mode
  test("compact - 90 seconds", () => {
    expect(duration(90, { compact: true })).toBe("1m 30s");
  });
  ...
  test("max_units 1 - 9000 seconds", () => {
    expect(duration(9000, { max_units: 1 })).toBe("3 hours");
  });
  ...
  test("compact and max_units 1", () => {
    expect(duration(3661, { compact: true, max_units: 1 })).toBe("1h");
  });
...
describe("parseDuration", () => {
  ...
  test("2 hours and 30 minutes", () => {
    expect(parseDuration("2 hours and 30 minutes")).toBe(9000);
  });
  ...
  // Decimal
  test("2.5 hours", () => {
    expect(parseDuration("2.5 hours")).toBe(9000);
  });
  ...
  test("2hrs", () => {
    expect(parseDuration("2hrs")).toBe(7200);
  });

  test("2 hour", () => {
    expect(parseDuration("2 hour")).toBe(7200);
  });
  ...
  test("30mins", () => {
    expect(parseDuration("30mins")).toBe(1800);
  });

  test("30 minute", () => {
    expect(parseDuration("30 minute")).toBe(1800);
  });

  test("45sec", () => {
    expect(parseDuration("45sec")).toBe(45);
  });

  test("45secs", () => {
    expect(parseDuration("45secs")).toBe(45);
  });

  test("45 second", () => {
    expect(parseDuration("45 second")).toBe(45);
  });
  ...
  test("2wk", () => {
    expect(parseDuration("2wk")).toBe(1209600);
  });

  test("2wks", () => {
    expect(parseDuration("2wks")).toBe(1209600);
  });

  test("2 weeks", () => {
    expect(parseDuration("2 weeks")).toBe(1209600);
  });
  ...
  test("1 day 2 hours 30 minutes", () => {
    expect(parseDuration("1 day 2 hours 30 minutes")).toBe(95400);
  });

  test("1w 2d", () => {
    expect(parseDuration("1w 2d")).toBe(777600);
  });
...
describe("humanDate", () => {
  ...
  test("today - earlier same day", () => {
    expect(humanDate(ref - 3600, ref)).toBe("Today");
  });

  test("yesterday", () => {
    expect(humanDate(ref - 86400, ref)).toBe("Yesterday");
  });

  test("tomorrow", () => {
    expect(humanDate(ref + 86400, ref)).toBe("Tomorrow");
  });

  test("last Sunday - 1 day ago", () => {
    expect(humanDate(ref - 86400, ref)).toBe("Yesterday"); // Sunday
  });

  test("last Saturday - 2 days ago", () => {
    expect(humanDate(ref - 2 * 86400, ref)).toBe("Last Saturday");
  });

  test("last Friday - 3 days ago", () => {
    expect(humanDate(ref - 3 * 86400, ref)).toBe("Last Friday");
  });

  test("last Monday - 7 days ago", () => {
    expect(humanDate(ref - 7 * 86400, ref)).toBe("Last Monday");
  });

  test("this Tuesday - 1 day from now", () => {
    expect(humanDate(ref + 86400, ref)).toBe("Tomorrow"); // Tuesday
  });

  test("this Wednesday - 2 days from now", () => {
    expect(humanDate(ref + 2 * 86400, ref)).toBe("This Wednesday");
  });

  test("this Sunday - 6 days from now", () => {
    expect(humanDate(ref + 6 * 86400, ref)).toBe("This Sunday");
  });

  test("same year - January 5", () => {
    const jan5 = 1704456000; // 2024-01-05
    expect(humanDate(jan5, ref)).toBe("January 5");
  });

  test("same year - March 20", () => {
    const mar20 = 1710892800; // 2024-03-20
    expect(humanDate(mar20, ref)).toBe("March 20");
  });

  test("same year - December 25", () => {
    const dec25 = 1735084800; // 2024-12-25
    expect(humanDate(dec25, ref)).toBe("December 25");
  });

  test("different year - future", () => {
    const jan1_2025 = 1735689600; // 2025-01-01
    expect(humanDate(jan1_2025, ref)).toBe("January 1, 2025");
  });

  test("different year - past", () => {
    const jan1_2023 = 1672531200; // 2023-01-01
