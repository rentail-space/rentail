...
export default function CityPage({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <main className="container mx-auto my-10 space-y-8 p-5">
      <title>{`Shopping Centers in ${city.name}, ${city.state.abbreviation} | Rentail.space`}</title>
      <meta
        name="description"
        content={`Find specialty leasing and short-term retail spaces in ${city.name}, ${city.state.abbreviation}. Browse ${centers.length} shopping centers with kiosks, pop-up shops, carts, and temporary storefronts. Real-time availability for seasonal and temporary retail opportunities.`}
      />
      <meta
        name="keywords"
